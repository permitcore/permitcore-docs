---
title: "Cohorts"
description: "The canonical 18-cohort taxonomy that PermitCore's pipeline classifies every permit into."
---
...
These are on the data-pipeline roadmap as future sub-cohort splits. Until
then, query the rolled-up parent and filter downstream by permit-type
keywords.

## Cohort assignment

Cohort tags are emitted by PermitCore's LLM-1 classifier per permit. Tags
are deterministic per permit (same input → same cohort), explainable
(reasoning surfaced via the future `/v1/permits/{id}/explanation` endpoint),
and improve over time as the classifier is fine-tuned on operator-validated
labels.
...
The permit-listing endpoint (`/v1/permits?cohort=…&jurisdiction=…`,
shipping ~3 weeks) lets you paginate the rows backing each count.
