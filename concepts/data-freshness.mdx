...
| ...and 6 more (see [`/v1/permits` reference](/api-reference/permits)) | |

DQ flags are pipeline-defined today and exposed via the upstream
recency artifact that powers the site's recency surfaces. They are
**not yet emitted on `GET /v1/permits`** — that emission lands with
the enrichment promotion (alongside `cohort` + the nested `pluto`
object). Watch the [changelog](/changelog) for the promotion
announcement.
