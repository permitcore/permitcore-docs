...
| Enterprise | From $599/mo | Custom | Custom |
...
All tiers include all 22 segments and full history.
...
| --- | --- | --- | --- |
| `type` | URI string | **YES** | Stable identifier. Pattern: `https://docs.permitcore.io/errors/{daily\|monthly}-{requests\|exports}-quota-exceeded`. Branch your error-handling on this URI, not on `title`/`detail`. |
...
<Note>
The legacy `legacy_code` field (value `"rate_limited"`) that earlier releases
emitted for pre-RFC-9457 clients was **removed on 2026-06-27**. Branch on the
`type` URI (and `quota_kind`) instead — it is no longer present in responses.
</Note>
...
- Move logic that branched on `error.code === "rate_limited"` to branching on
  the `type` URI instead.
...
See the [changelog](/changelog) for the removal note.
