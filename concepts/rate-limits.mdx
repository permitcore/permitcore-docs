...
| `legacy_code` | string | **DEPRECATED** — do not depend on it | Legacy `error.code` value (`rate_limited`) emitted for clients written against the pre-RFC-9457 envelope. It is being removed — **migrate to branching on `type`** and don't rely on this field. |
...
- Move any client logic branching on `error.code === "rate_limited"` to
  branching on `type` (URI) instead.
