...
- **`cohort`** — the buyer-targeting bucket the row classifies into, one of the 22 canonical cohorts. `null` = row not in the cohort matview OR in the matview but unsegmented. Identical taxonomy to `target_segment` on `/v1/permits/target`. See [Concepts → Cohorts](/concepts/cohorts).
...
`target_segment` accepts **all 22 canonical cohort slugs**. Behavior:
