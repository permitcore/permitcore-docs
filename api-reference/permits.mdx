...
- **`cohort`** — the buyer-targeting bucket the row classifies into, one of the 23 canonical segments. `null` = row not in the cohort matview OR in the matview but unsegmented. Identical taxonomy to `target_segment` on `/v1/permits/target`. See [Concepts → Cohorts](/concepts/cohorts).
...
- **Valid segment with zero rows in window:** `200 OK` with an empty
  ...
  the empty result.
- **Invalid slug** (not in the 23-segment enum): `422 Unprocessable Entity`
  ([`invalid-target-segment`](/errors/invalid-target-segment)) with a
  problem+json body enumerating the accepted whitelist.
- **Newer cohort caveat:** `multifamily_mep` is filterable and populated but
  **not yet certified to the ≥99% precision bar** of the other 22 segments —
  currently 3 metros (primarily Austin), largely historical. See the note on
  [Cohorts](/concepts/cohorts). The other 22 are the established, certified set.
