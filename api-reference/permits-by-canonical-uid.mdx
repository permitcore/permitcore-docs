...
Identical to `PermitResponse` on [`/v1/permits`](/api-reference/permits#response-shape) — 58 fields including the enrichment set (`cohort`, `dq_flags`, top-level PLUTO-derived fields, `description_llm_enriched`). The differences on this endpoint are:
