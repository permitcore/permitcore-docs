...
`cohort` value. All 23 slugs are listed in [Concepts → Cohorts](/concepts/cohorts).
...
- **`cohort`** — buyer-targeting bucket; one of the canonical
  ...
  grown to 23 — see the 2026-07-20 entry.)
...
- **`GET /v1/jurisdictions/{slug}/cohorts/distribution`** — per-jurisdiction
  cohort breakdown across the canonical segment taxonomy (18 at launch; 23
  today — see the 2026-07-20 entry). Cached 1 hour upstream
...
- **Segment classification** per permit via PermitCore's LLM-1
  classifier (18 segments at launch; 23 today). See [Concepts → Cohorts](/concepts/cohorts).
